...
namespace upm
{
  ...
  EdgeDrawer::EdgeDrawer(const LineDetectionExtraInfoPtr &gradientInfo,
                         ...
                         double junctionAngleTh) : lineFitThreshold(lineFitThreshold),
  ...
  void EdgeDrawer::drawEdgeTreeStack(Pixel anchor, ImageEdge &initialPixels, bool firstAnchorDirection)
  {
    ...
    while (!branchesStack.empty())
    {
      ...
      // std::cout << "outliersList.size() = " << outliersList.size() << std::endl;
      while (outliersList.size() <= UPM_MAX_OUTLIERS_TH)
      {
        ...
        if (!findNextPxWithGradient(gradDir, gradImg, imageWidth, imageHeight, px, lastPx))
        {
          ...
          // std::cout << "Stopping because the gradient level is 0 or the pixel is out of the image" << std::endl;
          break;
        }
        ...
        if (inlierOverwritten)
        {
          ...
          // std::cout << "Stopping because the pixel "
          //           << "px"
          //           << " is an "
          //           << (edgeImg[indexInArray] == UPM_ED_SEGMENT_INLIER_PX ? "INLIER" : "EDGE_PIXEL")
          //           << " and inlierOverwritten = " << inlierOverwritten << std::endl;
          break;
        }
        ...
        { // !segment: We have not yet adjusted a segment to the pixels
          ...
          if (pixels.size() - lastChekedPxIdx >= minLineLength)
          {
            ...
            // std::cout << "Trying to fit a segment, fitError: " << fitError << std::endl;
      ...
      // std::cout << "Drawing process has ended "
      //           << (segment ? "FOUND ONE SEGMENT???????????????????" : "WITHOUT SEGMENTS") << std::endl;
      if (segment)
      {
        cnt++;
      }
      ...
      if (segment)
      {
        ...
        if ((outliersList.size() > UPM_MAX_OUTLIERS_TH || inlierOverwritten) &&
            !wasExtended && treatJunctions && canSegmentBeExtended(*localSegment, true, extensionPixels))
        {
  ...
  void EdgeDrawer::drawEdgeInBothDirections(uint8_t direction, Pixel anchor)
  {
    ...
    // std::cout << " in FIRST direction: " << static_cast<int>(direction) << std::endl;
                                        ...
                                        ImageEdge &pixelsInTheExtension)
  {
    ...
    for (int GRAD_JUNCTION_SIZE : listOfJunctionSizes)
    {
      ...
      // Get the pixels in the extension using the Bresenham Algorithm4

      pixelsInTheExtension = upm::bresenham(firstPx.x, firstPx.y, px.x, px.y);
      //! noticed
      // std::cout << "pixelsInTheExtension.size() = " << pixelsInTheExtension.size() << std::endl;
      ...
      // std::cout << "fitsEigenvaluesCond:" << std::sqrt((s1 + s2) / (s1 - s2 + 0.00001)) << std::endl;
      ...
      // std::cout << "fitsAngleCond:" << circularDist(theta, eigen_angle, M_PI) << std::endl;
