#include <vector>
#include "FullSegmentInfo.h"
#include "camodocal/camera_models/CameraFactory.h"
#include "camodocal/camera_models/CataCamera.h"
#include "camodocal/camera_models/PinholeCamera.h"
#include "iostream"
using namespace std;
namespace upm
{

  FullSegmentInfo::FullSegmentInfo(const std::vector<Pixel> &pts) : pixels(&pts) {}

  FullSegmentInfo::FullSegmentInfo(const std::vector<Pixel> &pts, int startIdx) : pixels(&pts)
  {
    init(pts, startIdx);
  }
  // void eq_liftProjective(Eigen::Vector2d &p2d, Eigen::Vector3d &p3d)
  // {
  //   double lon;
  //   double lat;
  //   double left_sphere_up = M_PI / 180 * 90;
  //   double left_sphere_down = M_PI / 180 * 90;
  //   int new_rows = 512;
  //   int new_cols = 1024;
  //   double x = p2d(0);
  //   double y = p2d(1);
  //   lon = (x / double(new_cols) - 0.5) * (2 * M_PI);
  //   lat = -(y / double(new_rows) * (left_sphere_up + left_sphere_down) - left_sphere_up);
  //   Eigen::Vector3d tmp_p;
  //   tmp_p(2) = sin(lat);
  //   tmp_p(0) = cos(lat) * sin(lon);
  //   tmp_p(1) = cos(lat) * cos(lon);
  //   p3d = tmp_p;
  // }

  void FullSegmentInfo::init(const std::vector<Pixel> &pts, int startIdx)
  {
    assert(pts.size() > startIdx);
    assert(pts.size() >= 2);

    //! added by wz
    std::string config_file;
    config_file = "../config/mindvision.yaml";
    cv::FileStorage fsSettings(config_file, cv::FileStorage::READ);
    m_camera = camodocal::CameraFactory::instance()->generateCameraFromYamlFile(config_file.c_str());
    // width = fsSettings["image_width"];
    // height = fsSettings["image_height"];
    width = 1024;
    height = 512;
    // cout << "width: " << width << " height: " << height << endl;

    dx = pts.back().x - pts[startIdx].x;
    dy = pts.back().y - pts[startIdx].y;
    isHorizontal = UPM_ABS(dx) >= UPM_ABS(dy);

    // Fit the line
    leastSquareLineFit(pts, startIdx);
    //! changed by wz
    // leastSquareSphereLineFit(pts, startIdx);

    firstPx = pts[startIdx];
    lastPx = pts.back();
    prevFirstPx = pts[startIdx + 1];
    prevLastPx = pts[pts.size() - 2];
    firstPxIndex = startIdx;
    lastPxIndex = pts.size() - 1;

    arePixelsSorted = true;
    firstEndpointExtended = false;
    secondEndpointExtended = false;

    // for (int i = 0; i < width; i++)
    // {
    //   for (int j = 0; j < height; j++)
    //   {
    //     Eigen::Vector3d tmp_p;
    //     m_camera->liftProjective(Eigen::Vector2d(i, j), tmp_p);
    //     tmp_p.normalize();
    //     visionMap = cv::Mat::zeros(height, width, CV_32FC3);
    //     visionMap.at<cv::Vec3f>(j, i)[0] = tmp_p[0];
    //     visionMap.at<cv::Vec3f>(j, i)[1] = tmp_p[1];
    //     visionMap.at<cv::Vec3f>(j, i)[2] = tmp_p[2];
    //   }
    // }
  }

  void FullSegmentInfo::skipPositions()
  {
    static_assert(UPM_SKIP_EDGE_PT == 2, "Error this code is optimized for UPM_SKIP_EDGE_PT = 2");
    assert(pixels->size() == lastPxIndex + 1 + UPM_SKIP_EDGE_PT);

    // Remove the last UPM_SKIP_EDGE_PT of the model (here UPM_SKIP_EDGE_PT == 2)
    subtractPointFromModel(firstPx);
    subtractPointFromModel(prevFirstPx);
    //! changed by wz
    // subtractSpherePointFromModel(firstPx);
    // subtractSpherePointFromModel(prevFirstPx);

    // Update the indices of the pixels we are using in the vector of pixels. Move all the indices
    firstPxIndex += UPM_SKIP_EDGE_PT;
    lastPxIndex += UPM_SKIP_EDGE_PT;
    firstPx = (*pixels)[firstPxIndex];
    lastPx = pixels->back();
    prevFirstPx = (*pixels)[firstPxIndex + 1];
    prevLastPx = (*pixels)[lastPxIndex - 1];

    dx = lastPx.x - firstPx.x;
    dy = lastPx.y - firstPx.y;
    bool tmp = UPM_ABS(dx) >= UPM_ABS(dy);
    if (tmp != isHorizontal)
    {
      isHorizontal = tmp;
      // Change of orientation
      leastSquareLineFit(*pixels, firstPxIndex);
      //! changed by wz
      // leastSquareSphereLineFit(*pixels, firstPxIndex);
    }
    else
    {
      leastSquaresLineFitNewPoint(prevLastPx.x, prevLastPx.y);
      leastSquaresLineFitNewPoint(lastPx.x, lastPx.y);
      //! changed by wz
      // leastSquaresSphereLineFitNewPoint(prevLastPx.x, prevLastPx.y);
      // leastSquaresSphereLineFitNewPoint(lastPx.x, lastPx.y);
    }
  }

  void FullSegmentInfo::addPixel(int x, int y, int pixelIndexInEdge, bool isPixelAtTheEnd)
  {
    leastSquaresLineFitNewPoint(x, y);
    //! changed by wz
    // leastSquaresSphereLineFitNewPoint(x, y);
    assert(!std::isnan(equation[0]) && !std::isnan(equation[1]));

    if (isPixelAtTheEnd)
    {
      prevLastPx = lastPx;
      lastPx.x = x;
      lastPx.y = y;
    }
    else
    {
      prevFirstPx = firstPx;
      firstPx.x = x;
      firstPx.y = y;
    }

    // The list of pixels is not sorted but should be contiguous in memory.
    // In consequence the last pixel index is always the last added
    lastPxIndex = pixelIndexInEdge;
    arePixelsSorted = arePixelsSorted && isPixelAtTheEnd;
  }

  void FullSegmentInfo::removeLastPx(bool removeFromTheEnd)
  {
    // Subtract the pixel from the model parameters
    subtractPointFromModel(removeFromTheEnd ? lastPx : firstPx);
    //! changed by wz
    // subtractSpherePointFromModel(removeFromTheEnd ? lastPx : firstPx);
    // Update the endpoints information
    if (removeFromTheEnd)
    {
      assert(prevLastPx.x != -1);
      lastPx = prevLastPx;
      prevLastPx.x = -1;
      prevLastPx.y = -1;
    }
    else
    {
      assert(prevFirstPx.x != -1);
      firstPx = prevFirstPx;
      prevFirstPx.x = -1;
      prevFirstPx.y = -1;
    }
    lastPxIndex--;
  }

  void FullSegmentInfo::finish()
  {
    // Calculate the endpoints of the segment
    // calcSegmentEndpoints();
    //! changed by wz
    calcSphereSegmentEndpoints();
    assert(!std::isnan(endpoints[0]));
  }

  ImageEdge FullSegmentInfo::getPixels() const
  {
    assert(firstPxIndex < lastPxIndex);
    ImageEdge result;
    result.reserve(getNumOfPixels());
    // Copy the elements to the destination vector
    for (const Pixel &px : *this)
      result.push_back(px);
    return result;
  }

  // TODO change
  inline void FullSegmentInfo::calcSegmentEndpoints()
  {
    // At last, compute the line endpoints and store them. We project the first
    // and last pixels in the pixelChain onto the best fit line to get the line endpoints.
    // xp= (w2^2 * x0 -w1*w2 * y0 -w3 * w1)
    // yp= (w1^2 * y0 -w1*w2 * x0 -w3 * w2)
    float a1 = equation[1] * equation[1];
    float a2 = equation[0] * equation[0];
    float a3 = equation[0] * equation[1];
    float a4 = equation[2] * equation[0];
    float a5 = equation[2] * equation[1];
    // First pixel
    const Pixel &firstPx = getFirstPixel();
    int Px = firstPx.x;
    int Py = firstPx.y;
    endpoints[0] = a1 * Px - a3 * Py - a4; // x
    endpoints[1] = a2 * Py - a3 * Px - a5; // y
    // Last pixel
    const Pixel &secondPx = getLastPixel();
    Px = secondPx.x;
    Py = secondPx.y;
    endpoints[2] = a1 * Px - a3 * Py - a4; // x
    endpoints[3] = a2 * Py - a3 * Px - a5; // y
  }
  //! added by wz
  inline void FullSegmentInfo::calcSphereSegmentEndpoints()
  {
    // const Pixel &firstPx = getFirstPixel();
    // int Px = firstPx.x;
    // int Py = firstPx.y;
    // // TODO change
    // endpoints[0] = Px;
    // endpoints[1] = Py;
    // // Last pixel
    // const Pixel &secondPx = getLastPixel();
    // Px = secondPx.x;
    // Py = secondPx.y;
    // endpoints[2] = Px;
    // endpoints[3] = Py;
    // std::cout << "endpoints: " << endpoints[0] << " " << endpoints[1] << " " << endpoints[2] << " " << endpoints[3] << std::endl;

    const Pixel &firstPx = getFirstPixel();
    int Px, Py;
    Px = firstPx.x;
    Py = firstPx.y;
    // // if (horizontal)
    // // {
    // //   Px = firstPx.x;
    // //   Py = firstPx.y;
    // // }
    // // else
    // // {
    // //   Px = firstPx.y;
    // //   int Py = firstPx.x;
    // // }
    Eigen::Vector3d tmp;
    // m_camera->liftProjective(Eigen::Vector2d(Px, Py), tmp);
    eq_liftProjective(Eigen::Vector2d(Px, Py), tmp);
    tmp.normalize();
    // float a = equation[0];
    // float b = equation[1];
    // float c = equation[2];
    //! changed by wz !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    double a = equation_unit_shpere[0];
    double b = equation_unit_shpere[1];
    double c = equation_unit_shpere[2];
    // float a, b, c;
    // if (isHorizontal)
    // {
    //   a = equation[0];
    //   b = equation[1];
    //   c = equation[2];
    // }
    // else
    // {
    //   a = equation[1];
    //   b = equation[0];
    //   c = equation[2];
    // }
    Eigen::Vector3d new_tmp = Eigen::Vector3d(a, b, c).cross(tmp.cross(Eigen::Vector3d(a, b, c)));
    new_tmp.normalize();
    Eigen::Vector2d tmp_pixel;
    // m_camera->spaceToPlane(new_tmp, tmp_pixel);
    eq_spaceToPlane(new_tmp, tmp_pixel);
    endpoints[0] = tmp_pixel.x();
    endpoints[1] = tmp_pixel.y();

    // int pixel_threshold = 5;
    // if (sqrt((endpoints[0] - Px) * (endpoints[0] - Px) + (endpoints[1] - Py) * (endpoints[1] - Py)) > pixel_threshold)
    // {
    //   std::cout << "start    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!" << std::endl;
    //   std::cout << "tmp_pixel: " << tmp_pixel.x() << " " << tmp_pixel.y() << std::endl;
    //   std::cout << "Px: " << Px << " " << Py << std::endl;
    //   std::cout << "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!" << std::endl;
    //   endpoints[0] = Px;
    //   endpoints[1] = Py;
    // }
    // else
    // {
    //   std::cout << "start    -------------------------------------" << std::endl;
    //   std::cout << "tmp_pixel: " << tmp_pixel.x() << " " << tmp_pixel.y() << std::endl;
    //   std::cout << "Px: " << Px << " " << Py << std::endl;
    //   std::cout << "-------------------------------------" << std::endl;
    // }
    if (endpoints[0] < 0)
    {
      // std::cout << "endpoints[0]<0" << std::endl;
      endpoints[0] = 0;
    }
    if (endpoints[0] > width - 1)
    {
      // std::cout << "endpoints[0]>m_camera->w()-1" << std::endl;
      endpoints[0] = width - 1;
    }
    if (endpoints[1] < 0)
    {
      // std::cout << "endpoints[1]<0" << std::endl;
      endpoints[1] = 0;
    }
    if (endpoints[1] > height - 1)
    {
      // std::cout << "endpoints[1]>m_camera->h()-1" << std::endl;
      endpoints[1] = height - 1;
    }
    // Last pixel
    const Pixel &secondPx = getLastPixel();
    Px = secondPx.x;
    Py = secondPx.y;
    // m_camera->liftProjective(Eigen::Vector2d(Px, Py), tmp);
    eq_liftProjective(Eigen::Vector2d(Px, Py), tmp);
    tmp.normalize();
    new_tmp = Eigen::Vector3d(a, b, c).cross(tmp.cross(Eigen::Vector3d(a, b, c)));
    new_tmp.normalize();
    // m_camera->spaceToPlane(new_tmp, tmp_pixel);
    eq_spaceToPlane(new_tmp, tmp_pixel);
    endpoints[2] = tmp_pixel.x();
    endpoints[3] = tmp_pixel.y();

    // if (sqrt((endpoints[2] - Px) * (endpoints[2] - Px) + (endpoints[3] - Py) * (endpoints[3] - Py)) > pixel_threshold)
    // {
    //   std::cout << "end    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!" << std::endl;
    //   std::cout << "tmp_pixel: " << tmp_pixel.x() << " " << tmp_pixel.y() << std::endl;
    //   std::cout << "Px: " << Px << " " << Py << std::endl;
    //   std::cout << "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!" << std::endl;
    //   endpoints[2] = Px;
    //   endpoints[3] = Py;
    // }
    // else
    // {
    //   std::cout << "end    -------------------------------------" << std::endl;
    //   std::cout << "tmp_pixel: " << tmp_pixel.x() << " " << tmp_pixel.y() << std::endl;
    //   std::cout << "Px: " << Px << " " << Py << std::endl;
    //   std::cout << "-------------------------------------" << std::endl;
    // }

    if (endpoints[2] < 0)
    {
      // std::cout << "endpoints[2]<0" << std::endl;
      endpoints[2] = 0;
    }
    if (endpoints[2] > width - 1)
    {
      // std::cout << "endpoints[2]>m_camera->w()-1" << std::endl;
      endpoints[2] = width - 1;
    }
    if (endpoints[3] < 0)
    {
      // std::cout << "endpoints[3]<0" << std::endl;
      endpoints[3] = 0;
    }
    if (endpoints[3] > height - 1)
    {
      // std::cout << "endpoints[3]>m_camera->h()-1" << std::endl;
      endpoints[3] = height - 1;
    }
  }
  // TODO change (check)
  inline void FullSegmentInfo::leastSquareLineFit(const std::vector<Pixel> &pts, int startIdx)
  {
    int i, indpCoord, depCoord;
    sum_x_i = 0, sum_y_i = 0, sum_x_i_y_i = 0, sum_x_i_2 = 0;
    N = pts.size() - startIdx;

    //! added by wz
    sphere_sum_x_i_2 = 0, sphere_sum_x_i_y_i = 0, sphere_sum_x_i_z_i = 0;
    sphere_sum_y_i_2 = 0, sphere_sum_y_i_z_i = 0, sphere_sum_z_i_2 = 0;
    for (i = startIdx; i < pts.size(); i++)
    {
      double tmp_x, tmp_y, tmp_z;
      Eigen::Vector3d tmp;
      // m_camera->liftProjective(Eigen::Vector2d(pts[i].x, pts[i].y), tmp);
      eq_liftProjective(Eigen::Vector2d(pts[i].x, pts[i].y), tmp);
      tmp.normalize();
      tmp_x = tmp.x();
      tmp_y = tmp.y();
      tmp_z = tmp.z();
      sphere_sum_x_i_2 += tmp_x * tmp_x;
      sphere_sum_x_i_y_i += tmp_x * tmp_y;
      sphere_sum_x_i_z_i += tmp_x * tmp_z;
      sphere_sum_y_i_2 += tmp_y * tmp_y;
      sphere_sum_y_i_z_i += tmp_y * tmp_z;
      sphere_sum_z_i_2 += tmp_z * tmp_z;
    }
    // std::cout << "sphere_sum_x_i_z_i_1: " << sphere_sum_x_i_z_i << std::endl;
    calculateSphereLineEq();

    if (isHorizontal)
    {
      for (i = startIdx; i < pts.size(); i++)
      {
        indpCoord = pts[i].x;
        depCoord = pts[i].y;
        sum_x_i += indpCoord;
        sum_y_i += depCoord;
        sum_x_i_2 += indpCoord * indpCoord;
        sum_x_i_y_i += indpCoord * depCoord;
      }
    }
    else
    {
      for (i = startIdx; i < pts.size(); i++)
      {
        indpCoord = pts[i].y;
        depCoord = pts[i].x;
        sum_x_i += indpCoord;
        sum_y_i += depCoord;
        sum_x_i_2 += indpCoord * indpCoord;
        sum_x_i_y_i += indpCoord * depCoord;
      }
    }
    calculateLineEq();
  }
  //! added by wz
  inline void FullSegmentInfo::leastSquareSphereLineFit(const std::vector<Pixel> &pts, int startIdx)
  {
    int i, indpCoord, depCoord;
    sphere_sum_x_i_2 = 0, sphere_sum_x_i_y_i = 0, sphere_sum_x_i_z_i = 0;
    sphere_sum_y_i_2 = 0, sphere_sum_y_i_z_i = 0, sphere_sum_z_i_2 = 0;
    N = pts.size() - startIdx;
    if (isHorizontal)
    {
      for (i = startIdx; i < pts.size(); i++)
      {
        // indpCoord = pts[i].x;
        // depCoord = pts[i].y;
        double tmp_x, tmp_y, tmp_z;
        Eigen::Vector3d tmp;
        // m_camera->liftProjective(Eigen::Vector2d(pts[i].x, pts[i].y), tmp);
        eq_liftProjective(Eigen::Vector2d(pts[i].x, pts[i].y), tmp);
        tmp.normalize();
        tmp_x = tmp.x();
        tmp_y = tmp.y();
        tmp_z = tmp.z();
        sphere_sum_x_i_2 += tmp_x * tmp_x;
        sphere_sum_x_i_y_i += tmp_x * tmp_y;
        sphere_sum_x_i_z_i += tmp_x * tmp_z;
        sphere_sum_y_i_2 += tmp_y * tmp_y;
        sphere_sum_y_i_z_i += tmp_y * tmp_z;
        sphere_sum_z_i_2 += tmp_z * tmp_z;
      }
    }
    else
    {
      for (i = startIdx; i < pts.size(); i++)
      {
        // indpCoord = pts[i].y;
        // depCoord = pts[i].x;
        double tmp_x, tmp_y, tmp_z;
        Eigen::Vector3d tmp;
        // m_camera->liftProjective(Eigen::Vector2d(pts[i].x, pts[i].y), tmp);
        eq_liftProjective(Eigen::Vector2d(pts[i].x, pts[i].y), tmp);
        tmp.normalize();
        tmp_x = tmp.y();
        tmp_y = tmp.x();
        tmp_z = tmp.z();
        sphere_sum_x_i_2 += tmp_x * tmp_x;
        sphere_sum_x_i_y_i += tmp_x * tmp_y;
        sphere_sum_x_i_z_i += tmp_x * tmp_z;
        sphere_sum_y_i_2 += tmp_y * tmp_y;
        sphere_sum_y_i_z_i += tmp_y * tmp_z;
        sphere_sum_z_i_2 += tmp_z * tmp_z;
      }
    }

    calculateSphereLineEq();
  }
  // TODO change(check)
  inline void
  FullSegmentInfo::leastSquaresLineFitNewPoint(int x, int y)
  {
    const int indpCoord = isHorizontal ? x : y;
    const int depCoord = isHorizontal ? y : x;
    N++;
    sum_x_i += indpCoord;
    sum_y_i += depCoord;
    sum_x_i_2 += indpCoord * indpCoord;
    sum_x_i_y_i += indpCoord * depCoord;

    calculateLineEq();

    //! added by wz
    double tmp_x, tmp_y, tmp_z;
    Eigen::Vector3d tmp;
    // m_camera->liftProjective(Eigen::Vector2d(x, y), tmp);
    eq_liftProjective(Eigen::Vector2d(x, y), tmp);
    tmp.normalize();
    tmp_x = tmp.x();
    tmp_y = tmp.y();
    tmp_z = tmp.z();
    sphere_sum_x_i_2 += tmp_x * tmp_x;
    sphere_sum_y_i_2 += tmp_y * tmp_y;
    sphere_sum_z_i_2 += tmp_z * tmp_z;
    sphere_sum_x_i_y_i += tmp_x * tmp_y;
    sphere_sum_x_i_z_i += tmp_x * tmp_z;
    sphere_sum_y_i_z_i += tmp_y * tmp_z;
    // std::cout << "sphere_sum_x_i_z_i_2:" << sphere_sum_x_i_z_i << std::endl;
    calculateSphereLineEq();
  }
  //! added by wz
  inline void FullSegmentInfo::leastSquaresSphereLineFitNewPoint(int x, int y)
  {
    // const int indpCoord = isHorizontal ? x : y;
    // const int depCoord = isHorizontal ? y : x;
    N++;
    double tmp_x, tmp_y, tmp_z;
    Eigen::Vector3d tmp;
    // m_camera->liftProjective(Eigen::Vector2d(x, y), tmp);
    eq_liftProjective(Eigen::Vector2d(x, y), tmp);
    tmp.normalize();
    // tmp_x = tmp.x();
    // tmp_y = tmp.y();
    if (isHorizontal)
    {
      tmp_x = tmp.x();
      tmp_y = tmp.y();
    }
    else
    {
      tmp_x = tmp.y();
      tmp_y = tmp.x();
    }
    tmp_z = tmp.z();
    sphere_sum_x_i_2 += tmp_x * tmp_x;
    sphere_sum_y_i_2 += tmp_y * tmp_y;
    sphere_sum_z_i_2 += tmp_z * tmp_z;
    sphere_sum_x_i_y_i += tmp_x * tmp_y;
    sphere_sum_x_i_z_i += tmp_x * tmp_z;
    sphere_sum_y_i_z_i += tmp_y * tmp_z;

    calculateSphereLineEq();
  }
  // TODO change (check)
  inline void FullSegmentInfo::subtractPointFromModel(const Pixel &p)
  {
    const int independentCord = isHorizontal ? p.x : p.y;
    const int dependentCord = isHorizontal ? p.y : p.x;
    N--;
    sum_x_i -= independentCord;
    sum_y_i -= dependentCord;
    sum_x_i_2 -= independentCord * independentCord;
    sum_x_i_y_i -= independentCord * dependentCord;

    calculateLineEq();
    //! added by wz
    double tmp_x, tmp_y, tmp_z;
    Eigen::Vector3d tmp;
    // m_camera->liftProjective(Eigen::Vector2d(p.x, p.y), tmp);
    eq_liftProjective(Eigen::Vector2d(p.x, p.y), tmp);
    tmp.normalize();
    tmp_x = tmp.x();
    tmp_y = tmp.y();
    tmp_z = tmp.z();
    sphere_sum_x_i_2 -= tmp_x * tmp_x;
    sphere_sum_y_i_2 -= tmp_y * tmp_y;
    sphere_sum_z_i_2 -= tmp_z * tmp_z;
    sphere_sum_x_i_y_i -= tmp_x * tmp_y;
    sphere_sum_x_i_z_i -= tmp_x * tmp_z;
    sphere_sum_y_i_z_i -= tmp_y * tmp_z;
    // std::cout << "sphere_sum_x_i_z_i_3:" << sphere_sum_x_i_z_i << std::endl;
    calculateSphereLineEq();
  }
  //! added by wz
  inline void FullSegmentInfo::subtractSpherePointFromModel(const Pixel &p)
  {
    // const int independentCord = isHorizontal ? p.x : p.y;
    // const int dependentCord = isHorizontal ? p.y : p.x;
    N--;
    double tmp_x, tmp_y, tmp_z;
    Eigen::Vector3d tmp;
    // m_camera->liftProjective(Eigen::Vector2d(p.x, p.y), tmp);
    eq_liftProjective(Eigen::Vector2d(p.x, p.y), tmp);
    // tmp_x = tmp.x();
    // tmp_y = tmp.y();
    if (isHorizontal)
    {
      tmp_x = tmp.x();
      tmp_y = tmp.y();
    }
    else
    {
      tmp_x = tmp.y();
      tmp_y = tmp.x();
    }
    tmp_z = tmp.z();
    tmp.normalize();

    sphere_sum_x_i_2 -= tmp_x * tmp_x;
    sphere_sum_y_i_2 -= tmp_y * tmp_y;
    sphere_sum_z_i_2 -= tmp_z * tmp_z;
    sphere_sum_x_i_y_i -= tmp_x * tmp_y;
    sphere_sum_x_i_z_i -= tmp_x * tmp_z;
    sphere_sum_y_i_z_i -= tmp_y * tmp_z;
    calculateSphereLineEq();
  }
  // TODO change (check)
  inline void FullSegmentInfo::calculateLineEq()
  {
    // Line equation is calculated as:
    // 	\[
    //	ax + by + c = 0 \left\{
    //	\begin{array}{ll}
    //	a = N \cdot \sum (x_i y_i) - \sum x_i \cdot \sum y_i\\
  //	b = \left( \sum x_i \right )^2 - N \cdot \sum x_i^2\\
  //	c = \sum y_i \cdot \sum x_i^2 - \sum x_i \cdot \sum x_i y_i
    //	\end{array}
    //	\right.
    //	\]
    // Compute the line equation coefficients
    equation[2] = sum_y_i * sum_x_i_2 - sum_x_i * sum_x_i_y_i;
    if (isHorizontal)
    {
      equation[0] = N * sum_x_i_y_i - sum_x_i * sum_y_i;
      equation[1] = sum_x_i * sum_x_i - N * sum_x_i_2;
    }
    else
    {
      equation[1] = N * sum_x_i_y_i - sum_x_i * sum_y_i;
      equation[0] = sum_x_i * sum_x_i - N * sum_x_i_2;
    }

    // Normalize the line equation
    equation *= (1 / std::sqrt(equation[0] * equation[0] + equation[1] * equation[1]));

    // Fix the line equation
    if (dx * -equation[1] + dy * equation[0] < 0)
    {
      equation[0] = -equation[0];
      equation[1] = -equation[1];
      equation[2] = -equation[2];
    }

    // std::cout << "isHorizontal: " << isHorizontal << std::endl;
    // std::cout << "equation: " << std::endl
    //           << equation << std::endl;
  }
  //! added by wz
  // inline void FullSegmentInfo::calculateSphereLineEq()
  // {
  //   equation[2] = sphere_sum_y_i_z_i * sphere_sum_x_i_2 - sphere_sum_x_i_z_i * sphere_sum_x_i_y_i;
  //   if (isHorizontal)
  //   {
  //     equation[0] = sphere_sum_z_i_2 * sphere_sum_x_i_y_i - sphere_sum_x_i_z_i * sphere_sum_y_i_z_i;
  //     equation[1] = sphere_sum_x_i_z_i * sphere_sum_x_i_z_i - sphere_sum_z_i_2 * sphere_sum_x_i_2;
  //   }
  //   else
  //   {
  //     equation[1] = sphere_sum_z_i_2 * sphere_sum_x_i_y_i - sphere_sum_x_i_z_i * sphere_sum_y_i_z_i;
  //     equation[0] = sphere_sum_x_i_z_i * sphere_sum_x_i_z_i - sphere_sum_z_i_2 * sphere_sum_x_i_2;
  //   }
  //   equation *= (1 / std::sqrt(equation[0] * equation[0] + equation[1] * equation[1] + equation[2] * equation[2]));
  //   // if (dx * -equation[1] + dy * equation[0] + dz * equation[2] < 0)
  //   // {
  //   //   equation[0] = -equation[0];
  //   //   equation[1] = -equation[1];
  //   //   equation[2] = -equation[2];
  //   // }
  //   // std::cout << "isHorizontal: " << isHorizontal << std::endl;
  //   // std::cout << "equation: " << std::endl
  //   //           << equation << std::endl;
  // }
  //! added by wz
  inline void FullSegmentInfo::calculateSphereLineEq()
  {
    equation_unit_shpere[2] = sphere_sum_y_i_z_i * sphere_sum_x_i_2 - sphere_sum_x_i_z_i * sphere_sum_x_i_y_i;
    equation_unit_shpere[0] = sphere_sum_z_i_2 * sphere_sum_x_i_y_i - sphere_sum_x_i_z_i * sphere_sum_y_i_z_i;
    equation_unit_shpere[1] = sphere_sum_x_i_z_i * sphere_sum_x_i_z_i - sphere_sum_z_i_2 * sphere_sum_x_i_2;
    // std::cout << "sphere_sum_x_i_z_i: " << sphere_sum_x_i_z_i << std::endl;
    // std::cout << "equation_unit_shpere: " << equation_unit_shpere << std::endl;
    equation_unit_shpere *= (1 / std::sqrt(equation_unit_shpere[0] * equation_unit_shpere[0] + equation_unit_shpere[1] * equation_unit_shpere[1] + equation_unit_shpere[2] * equation_unit_shpere[2]));

    // Eigen::MatrixXd A(3, 3);
    // A << sphere_sum_x_i_2, sphere_sum_x_i_y_i, sphere_sum_x_i_z_i,
    //     sphere_sum_x_i_y_i, sphere_sum_y_i_2, sphere_sum_y_i_z_i,
    //     sphere_sum_x_i_z_i, sphere_sum_y_i_z_i, sphere_sum_z_i_2;
    // Eigen::JacobiSVD<Eigen::MatrixXd> svd(A, Eigen::ComputeFullU | Eigen::ComputeFullV);
    // Eigen::Vector3d solve_x;
    // solve_x = svd.matrixV().col(2);
    // solve_x.normalize();
    // equation_unit_shpere = solve_x;

    // if (dx * -equation[1] + dy * equation[0] + dz * equation[2] < 0)
    // {
    //   equation[0] = -equation[0];
    //   equation[1] = -equation[1];
    //   equation[2] = -equation[2];
    // }
    // std::cout << "isHorizontal: " << isHorizontal << std::endl;
    // std::cout << "equation: " << std::endl
    //           << equation << std::endl;
  }

}
